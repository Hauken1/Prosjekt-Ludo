...
public class Player {
	...
	private void loginChecker(String username, String pass) throws IOException {
		...
		if (username.startsWith("SENDLOGIN:") && pass.startsWith("SENDLOGIN:")) {
			boolean login = true;	//m� skiftes til false n�r databasen har funksjoner for login
		...
		// login = DatabaseTest.registerNewUser(user, pw);
			...
			boolean register = true; //m� skiftes til false n�r databasen har funksjoner for registering.
